...
class MessagesController < ApplicationController
  ...
  before_action :ensure_related_user, only: %i[show]
  before_action :ensure_sent_user, only: %i[edit update destroy]
  ...
  def ensure_sent_user
    redirect_to(:users, notice: '権限がありません') unless @message.sent_by?(@current_user)
  end

  def ensure_related_user
    redirect_to(:users, notice: '権限がありません') unless @message.related?(@current_user)
  end
end
